...
with lib; let
  ...
  cfg = config.jeomod.applications.firefox;
  hmConfig = config.home-manager.users.${config.jeomod.user};
in {
  options.jeomod.applications.firefox = {
    enable = mkEnableOption "Firefox";
  };

  config = mkIf cfg.enable {
    home-manager.users.${config.jeomod.user} = {
      home.sessionVariables = mkIf hmConfig.wayland.windowManager.hyprland.enable {
    ...
    jeomod.system.impermanence.user.directories = [".mozilla/firefox/default"];
  };
}
